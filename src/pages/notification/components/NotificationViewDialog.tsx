...
const NotificationViewDialog: React.FC<NotificationViewDialogProps> = ({
  ...
  const TypeIcon = notification ? getTypeIcon(notification.type as string) : Bell;
  const typeStyle = notification ? getTypeColor(notification.type as string) : null;

  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="NotificationResponseDto Details"
      size="lg"
    >
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-12 h-12 rounded-md flex items-center justify-center ${typeStyle?.bg}`}
              >
        ...
        <p className="text-center py-4 text-[var(--text-secondary)]">
          NotificationResponseDto not found.
        </p>
      )}
    </Modal>
  );
