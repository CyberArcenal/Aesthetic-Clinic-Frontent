...
const AppointmentsPage: React.FC = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    showLoading("Deleting...");
    try {
      await Promise.all(selectedAppointments.map(id => appointmentApi.deleteAppointment(id)));
      showToast(`${selectedAppointments.length} appointments deleted`, "success");
      setSelectedAppointments([]);
      reload();
    } catch (err: any) {
      dialogs.alert({ title: "Error", message: err.message });
  ...
  const handleExportCSV = async () => {
    ...
    try {
      const headers = ["ID", "Client", "Treatment", "Date/Time", "Status", "Staff", "Notes"];
  ...
  return (
    <div className="compact-card rounded-md shadow-md border" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border-color)" }}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4 p-4">
        <div>
          <h2 className="text-base font-semibold" style={{ color: "var(--sidebar-text)" }}>Appointments</h2>
          <p className="text-sm mt-0.5" style={{ color: "var(--text-secondary)" }}>Manage clinic appointments</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button className="compact-button rounded-md flex items-center gap-1" onClick={() => setShowFilters(!showFilters)} style={{ backgroundColor: "var(--card-secondary-bg)", color: "var(--sidebar-text)" }}>
          ...
          <Button onClick={handleExportCSV} disabled={exportLoading || appointments.length === 0} className="compact-button rounded-md flex items-center gap-1">
            <Download className="icon-xs" /> {exportLoading ? "..." : "Export CSV"}
          </Button>
          <Button onClick={formDialog.openAdd} variant="success" size="sm" icon={Plus} iconPosition="left">Book Appointment</Button>
        </div>
      </div>
      ...
      {appointments.length > 0 && (
        <div className="mx-4 mb-4 compact-card rounded-md border p-3 flex flex-wrap items-center justify-between" style={{ backgroundColor: "var(--card-secondary-bg)", borderColor: "var(--border-color)" }}>
          <div className="flex items-center gap-2 text-xs">
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[var(--accent-green)]"></span>Total: {pagination.totalCount}</span>
          </div>
          <div className="text-xs" style={{ color: "var(--text-secondary)" }}>Showing {start} to {end} of {pagination.totalCount} appointments</div>
        </div>
      )}
      ...
      {selectedAppointments.length > 0 && (
        <div className="mx-4 mb-2 compact-card rounded-md border p-2 flex items-center justify-between" style={{ backgroundColor: "var(--accent-blue-light)", borderColor: "var(--accent-blue)" }}>
          <span className="font-medium text-sm">{selectedAppointments.length} appointment(s) selected</span>
      ...
      {loading ? (
        ...
        <>
          <AppointmentTable
            ...
            onView={(appointment) => {viewDialog.open(appointment.id)}}
            onEdit={formDialog.openEdit}
            onDelete={handleDelete}
          />
          {appointments.length === 0 && (
            <div className="text-center py-8 border rounded-md" style={{ borderColor: "var(--border-color)" }}>
              <CalendarDays className="icon-xl mx-auto mb-2" style={{ color: "var(--text-secondary)" }} />
              <p className="text-base" style={{ color: "var(--sidebar-text)" }}>No appointments found.</p>
              <button onClick={formDialog.openAdd} className="mt-2 btn btn-primary btn-sm">Book First Appointment</button>
            </div>
          )}
