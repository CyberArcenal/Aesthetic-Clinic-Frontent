...
const UsersPage: React.FC = () => {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      const headers = ["ID", "Username", "Full Name", "Email", "Roles", "Active", "Created At"];
    ...
    <div className="compact-card rounded-md shadow-md border" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border-color)" }}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button onClick={handleExportCSV} disabled={exportLoading || users.length === 0} className="compact-button rounded-md flex items-center gap-1">
      ...
      {users.length > 0 && (
        <div className="mx-4 mb-4 compact-card rounded-md border p-3 flex flex-wrap items-center justify-between" style={{ backgroundColor: "var(--card-secondary-bg)", borderColor: "var(--border-color)" }}>
          ...
          <div className="text-xs" style={{ color: "var(--text-secondary)" }}>Showing {start} to {end} of {pagination.totalCount} users</div>
        </div>
      )}

      {/* Filters */}
      {showFilters && <UserFilterBar filters={filters} onFilterChange={handleFilterChange} onReset={resetFilters} roles={rolesList} />}
      ...
      {selectedUsers.length > 0 && (
        <div className="mx-4 mb-2 compact-card rounded-md border p-2 flex items-center justify-between" style={{ backgroundColor: "var(--accent-blue-light)", borderColor: "var(--accent-blue)" }}>
          <span className="font-medium text-sm">{selectedUsers.length} user(s) selected</span>
        ...
        <>
          ...
          {users.length === 0 && (
            <div className="text-center py-8 border rounded-md" style={{ borderColor: "var(--border-color)" }}>
              ...
              <p className="text-base" style={{ color: "var(--sidebar-text)" }}>No users found.</p>
              <button onClick={formDialog.openAdd} className="mt-2 btn btn-primary btn-sm">Add First User</button>
            </div>
          )}
