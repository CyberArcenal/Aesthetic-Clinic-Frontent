...
const PaymentsPage: React.FC = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        selectedPayments.map((id) => billingApi.deletePayment(id)),
      );
      ...
      {loading ? (
        ...
        <>
          <PaymentTable
            ...
            onSort={(key) => {
              handleSort(key as keyof (typeof paginatedPayments)[0]);
            }}
            sortConfig={sortConfig}
            onView={(payment) => {
              viewDialog.open(payment.id);
            }}
