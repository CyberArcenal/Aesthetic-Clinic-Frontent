...
const StaffPage: React.FC = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        selectedStaff.map((id) => staffApi.deleteStaffMember(id)),
      );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            disabled={exportLoading || staff.length === 0}
            className="compact-button rounded-md flex items-center gap-1"
          >
      ...
      {staff.length > 0 && (
        ...
        >
          ...
          <div className="text-xs" style={{ color: "var(--text-secondary)" }}>
            Showing {start} to {end} of {pagination.totalCount} staff
          </div>
        </div>
      )}
