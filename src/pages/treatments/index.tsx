...
const TreatmentsPage: React.FC = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    showLoading("Deleting...");
    try {
      await Promise.all(selectedTreatments.map((id) => treatmentApi.deleteTreatment(id)));
  ...
  const handleExportCSV = async () => {
    ...
    try {
      const headers = ["ID", "Name", "Category", "Duration (min)", "Price", "Active"];
  ...
  return (
    <div className="compact-card rounded-md shadow-md border" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border-color)" }}>
      ...
      {/* Filters */}
      {showFilters && <TreatmentFilterBar filters={filters} onFilterChange={handleFilterChange} onReset={resetFilters} categories={categories as string[]} />}
      ...
      {selectedTreatments.length > 0 && (
        ...
        >
          <span className="font-medium text-sm">{selectedTreatments.length} treatment(s) selected</span>
        ...
        <>
          ...
          {treatments.length === 0 && (
            <div className="text-center py-8 border rounded-md" style={{ borderColor: "var(--border-color)" }}>
              <Stethoscope className="icon-xl mx-auto mb-2" style={{ color: "var(--text-secondary)" }} />
              <p className="text-base" style={{ color: "var(--sidebar-text)" }}>No treatments found.</p>
